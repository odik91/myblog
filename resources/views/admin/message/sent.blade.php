...
<script>
  ...
  let indexOffset = 0;
  let count = {{ count(App\Models\Message::where('status', 'sent')->get()) }};
    
  // console.log(count);
  let baseURL = "{{ route('message.index') }}";
  let routing = "{{ route('message.nextSentPage') }}";
  ...
  $("#delete").click(() => {
    ...
    checkedInput('delete', 'sent', routingCheckedInput);
    refresh(baseURL, routing, "{{ route('message.inboxCount') }}", 'sent');
  });
  ...
  $('#read').click(() => {
    let linking = "{{ route('message.markRead') }}";
    if ($('input:checkbox[name=selection]:checked').length > 0) {
      read(linking);
    }
    refresh(baseURL, routing, "{{ route('message.inboxCount') }}", 'sent');
  ...
  $('#reload').click(() => {
    ...
    refresh(baseURL, routing, targetCountRoute, 'sent');
  });  
  ...
  //   let baseURL = "{{ route('message.index') }}";
  //   $('#next').click(() => {
  //     nextPrev('next', routing, baseURL);      
  //     $('#mark').html("<i class=\"far fa-square\"></i>");
  //     i = 0;
  //   });

  //   $('#previous').click(() => {
  //     nextPrev('previous', routing, baseURL);  
  //     $('#mark').html("<i class=\"far fa-square\"></i>");
  //     i = 0;
  //   })
  // });
  ...
  //   read(routing);
  //   reloding(baseURL, "{{ route('message.nextSentPage') }}", "{{ route('message.inboxCount') }}", 'sent');
  // });
  ...
  //   let routing = "{{ route('message.nextSentPage') }}";
  //   let targetCountRoute = "{{ route('message.inboxCount') }}";
  //   reloding(baseURL, routing, targetCountRoute, 'sent');
  // });  
</script>
@endpush