              ...
              <table class="table table-hover table-striped">
                <tbody id="mailing">
                  ...
                  <tr id="listingClicked">
                    ...
                    <td class="mailbox-name"><a href="{{ route('message.read', [$message['id']]) }}"
                        class="text-dark">{{ ucwords($message['name']) }}</a></td>
                    <td class="mailbox-subject">
                      <a href="{{ route('message.read', [$message['id']]) }}" class="text-dark">
                        @php
...
<script>
  ...
  let indexOffset = 0;
  let count = {{ count(App\Models\Message::where('drafts', '!=', 'drafts')->where('status', '!=', 'sent')->orderBy('created_at', 'desc')->get()) }};
    
  // console.log(count);
  let baseURL = "{{ route('message.index') }}";
  let routing = "{{ route('message.nextPageInbox') }}";
  ...
  $("#delete").click(() => {
    ...
    if ($('input:checkbox[name=selection]:checked').length > 0) {
      checkedInput('delete', 'inbox', routingCheckedInput);
    }
    refresh(baseURL, routing, "{{ route('message.inboxCount') }}", 'inbox');
  });
